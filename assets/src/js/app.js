...
(function() {
  ...
  dependencies = ['ajoslin.mobile-navigate', 'snap', '$app.services', '$app.filters', '$app.directives', 'autoresize', 'loadfocus', 'fn.pullToRefresh', 'fn.swipeable'];
  ...
  angular.module('$app').controller('NotesCtrl', function($scope, $notes, $user, $navigate, $timeout, forgeUI) {
    ...
    populateTagLists = function() {
      $notes.getTags().then(function(list) {
        return $scope.hashtags = list;
      });
      return $notes.getContacts().then(function(list) {
        return $scope.attags = list;
      });
    };
    populateTagLists();
    ...
    $scope.addSearchFilter = function(text) {
    ...
    return $scope.removeFilter = function(index) {
      $scope.selectedFilters[index].data.selected = false;
      return $scope.selectedFilters.splice(index, 1);
    };
  });

  angular.module('$app').controller('NewNoteCtrl', function($scope, $navigate, $utils, $notes, $contacts) {
    $scope.noteText = '';
    ...
    (function() {
      var developingList;
      developingList = null;
      return $notes.getTags().then(function(list) {
        developingList = list;
        return $notes.getContacts();
      }).then(function(list) {
        return $scope.tags = developingList.concat(list);
      });
    })();
    $scope.save = function() {
      return $notes.save({
  ...
  angular.module('$app').controller('ViewNoteCtrl', function($scope, $navigate, $routeParams, $utils, $notes) {
    $scope.note = {};
  ...
  angular.module('$app').controller('SignUpEmailCtrl', function($scope, $navigate, $user) {
    $scope.back = function() {
      return $navigate.back();
    };
    return $scope.signUp = function(form) {
      ...
      return promise.then(function(user) {
        ...
        return $scope.errorMessage = "Some error message that definitely needs to be changed";
      });
    };
  });

  angular.module('$app').controller('SignInCtrl', function($scope, $navigate, $user) {
    $scope.back = function() {
      return $navigate.back();
    };
    return $scope.signIn = function(form) {
      var promise;
      promise = $user.signIn(angular.copy(form));
  ...
  angular.module('$app.directives').directive('autocomplete', function() {
  ...
  }).directive('autocompleteText', function() {
    return {
      restrict: 'A',
      link: function(scope, element, attrs, $log) {
        var getCurrentWord, getCurrentWordIndices, getCursorPosition, getNextWordBreak, setCursorPosition, updateCurrentWord, whitespace;
        ...
        updateCurrentWord = function() {
          return scope.currentWord = getCurrentWord(element.val(), getCursorPosition(element));
        };
    ...
    return {
      restrict: 'E',
      template: '<button ng-hide="currentWord[0] === \'#\' || currentWord[0] === \'@\'"></button>',
      replace: true,
  ...
  angular.module('$app.services').service('$forge', [
     function() {
      ...
      $forge = {};
      noForge = function(defered, method, args) {
        return setTimeout(function() {
          return $root.$apply(function() {
            return $fake.call(defered, method, args);
          });
        }, 100);
      };
      ...
      moduleCall = function(name, args) {
        var argsExist, baseForge, defered, error, method, module0, module1, namespace, success, tripleNamespaced, _ref;
  ...
  angular.module('$app.services').service('forgeUI', [
    '$forge', function($forge) {
      ...
      return this.killFormAssist = getPluginCall('kill_form_assist.killBar');
    }
  ]);
  ...
  angular.module('$app.services').service('$notesServer', function($kinvey) {
    var getQuery, getSortedQuery, notes;
    notes = {};
  ...
  angular.module('$app.services').service('$notes', function($notesServer, $db, $utils, $rootScope) {
    ...
    notes.harmonize = function() {
      var firstLogin, latestModifiedTime, noteServerOptions;
      latestModifiedTime = $db.lastSync();
      ...
      return $notesServer.get(noteServerOptions).then(function(notesFromServer) {
        ...
        return $db.sync(notesFromServer);
      }).then(function() {
        if (latestModifiedTime) {
          $db.lastSync(latestModifiedTime);
        }
  ...
  angular.module('$app.services').service('forgeDB', [
    '$forge', function($forge) {
      var getPluginCall;
      getPluginCall = $forge.getPluginCall;
      this.dbWriteAll = getPluginCall('database.writeAll');
      this.dbMultiQuery = getPluginCall('database.multiQuery');
      this.dbDropTables = getPluginCall('database.dropTables');
      this.dbQuery = getPluginCall('database.query');
      return this.dbCreateTables = getPluginCall('database.createTables');
    }
  ]);
  ...
  angular.module('$app.services').service('nativeInterface', [
    'queryBuilder', '$utils', 'forgeDB', '$user', function(qb, _, $forge, $user) {
      var NativeInterface;
      NativeInterface = (function() {
        ...
        NativeInterface.prototype.update = function(model, options) {
          ...
          return this._makeAndCallQuery(model, options, angular.bind(this.queryBuilder, this.queryBuilder.updateNote), 'update').then(function() {
            ...
            return _this._writeAllQueries(_.flatten(bigArray));
          });
        };
        ...
        NativeInterface.prototype.sync = function(models, options) {
          ...
          return $forge.dbMultiQuery({
            queries: _.map(models, angular.bind(this.queryBuilder, this.queryBuilder.fetchSingleNote))
        ...
        NativeInterface.prototype.getNote = function(id) {
          var args, q;
          q = Q.defer();
          args = {
        ...
        NativeInterface.prototype._getStuff = function(args) {
          ...
          return $forge.dbQuery({
            ...
            return dbData.map(function(note) {
              if (note.id === 'undefined') {
                delete note.id;
              }
              delete note.stash_bool;
              if (note[this.STASH]) {
        ...
        NativeInterface.prototype._writeFromServer = function(options) {
          ...
          sorted = _.sortBy(this._arraysToObjects('model', models, 'result', results), function(object) {
            if (isNew(object)) {
              return 0;
            } else {
              return 42;
          ...
          this._appendStashBool(compactSorted);
          return this._writeAllQueries(_.flatten((function() {
            var _i, _len, _results;
            _results = [];
            ...
            return _this._writeAllQueries(_.flatten(entityQueries));
          }, dbError);
        };

        NativeInterface.prototype._appendStashBool = function(compactSorted) {
          var CURRENT_ID,
            _this = this;
          CURRENT_ID = null;
          return compactSorted.forEach(function(obj) {
            return obj.model[_this.STASH_BOOL] = _this._getStashBool(obj.model, CURRENT_ID);
          });
        };

        NativeInterface.prototype._getStashBool = function(model, id) {
          if (model[this.STASH]) {
            return !!model[this.STASH][id];
          } else {
            return false;
          }
        };
    ...
    'dbLiterals', '$utils', function(dbLiterals, utils) {
      ...
      QueryBuilder = (function() {
        ...
        QueryBuilder.prototype.addNote = function(model) {
          return ("insert into Notes (" + this.TEXT + "," + this.ID + ",") + ("" + this.TIMESTAMP + "," + this.STATUS + ", " + this.STASH + ", " + this.STASH_BOOL + ") ") + (" values (?,'" + (this._getID(model)) + "',") + ("'" + (this._get(model, this.TIMESTAMP)) + "',") + (" '" + (this._get(model, this.STATUS)) + "',") + ("'" + (JSON.stringify(this._get(model, this.STASH))) + "',") + ("'" + (this._get(model, this.STASH_BOOL)) + "')");
        };
        ...
        QueryBuilder.prototype.fetchSingleNote = function(model) {
          return "select * from notes where id='" + (this._getID(model)) + "'";
        };
  ...
  angular.module('$app.services').service('testingUtilities', [
    '$utils', function(utils) {
      var badFindOneWhere, badFindWhere, checkContains, curriedFindWhere, newLocalID, notes, pickRandom, randomStashStuff;
      newLocalID = (function() {
        var i;
        i = 0;
        return function() {
  ...
  angular.module('$app.filters').filter('HTMLize', function() {
    ...
    return function(text, notClickable) {
      var i, line, lines, _i, _len;
      if (notClickable == null) {
        notClickable = false;
      }
